...
router.post('/register', (req, res, next) => {  
  ...
  pool.query(queryText, [name, authorization_id])
    .then((result) => { 
      const conpany_id = result;
      const username = req.body.username;
      // const password = encryptLib.encryptPassword(req.body.password);
      const password = 'tuna';
      const queryText2 = 'INSERT INTO users (username, password) WHERE company_id=$1 VALUES ($1, $2, $3)';
      pool.query(queryText2, [username, password, conpany_id])
      res.sendStatus(201); 
    })
    .catch((err) => { next(err); });
});
